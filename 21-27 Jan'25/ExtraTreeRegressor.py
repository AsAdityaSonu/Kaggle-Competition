...
# Cross-validation for Extra Trees Regressor with tqdm progress bar
...
for train_index, val_index in tqdm(kf.split(x_train_poly), total=kf.get_n_splits(), desc="Cross-validation"):
    ...
    etr_model.fit(X_train_fold, y_train_fold)
    fold_score = etr_model.score(X_val_fold, y_val_fold)
...
etr_model.fit(x_train_poly, y_train)

# Evaluate on training data
train_r2 = r2_score(y_train, etr_model.predict(x_train_poly))
...
test_predictions = etr_model.predict(x_test_poly)
